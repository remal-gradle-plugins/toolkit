...
public abstract class XmlUtils {
     ...
     * <p>This method parses XML content with Saxon library.</p>
     */
    @SneakyThrows
    public static Document parseXml(Path path) {
        path = normalizePath(path);
        try (val inputStream = newInputStream(path)) {
            return DOCUMENT_BUILDER.parse(inputStream, path.toString());
        }
    }
    ...
    static {
        val documentBuilder = new DocumentBuilderImpl();
